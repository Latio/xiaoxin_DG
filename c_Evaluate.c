...
void evaluateHLLFunc(const double hmin, ///< water threshold
	...
	#if 0
	printf("hm=%f, qnM=%f, qvM=%f, Em=[%f, %f, %f], Gm=[%f, %f, %f]\n", hM,
		qnM, qvM, Em[0], Em[1], Em[2], Gm[0], Gm[1], Gm[2]);
	printf("hp=%f, qnP=%f, qvP=%f, Ep=[%f, %f, %f], Gp=[%f, %f, %f]\n", hP,
...
{
	FluxSolver solver = ConvertInputMexVariable2d(hmin_, gra_, nx_, ny_, fm_, fp_, TNfp_, K_);

	const int TNfp = solver.TNfp;
	const int K = solver.K;
	double *Fh = fluxS_;
